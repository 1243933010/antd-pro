...
const MaterialLibrary: React.FC = () => {
    ...
    const deleteImg = async () => {
        ...
        let result = await delImg(delInfo);
        if (result.code === 0) {
            message.success({ content: result.message })
            ...
            <div className='img-flex'>
                {
                    data.length > 0 && data.map((val: { url: string }, index) => (
                        <div className='img-size' key={index}>
                            ...
                            {mouseIndex == index && <div onMouseOut={() => { setMouseIndex(null) }} onClick={() => openDelModal(val)} className='popup'><DeleteTwoTone style={{ fontSize: '30px' }} /></div>}
                        </div>
                    ))
                }
            </div>
            {pagination.total>0&&<div  className='Pagination'> <Pagination defaultCurrent={pagination.current} pageSize={pagination.pageSize} total={pagination.total} onChange={paginationChange} /></div>}
